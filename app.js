(() => {
  ...
  async function waitStopped(space,path,labels) {
    ...
    const deadline = Date.now() + 180000;
    while (Date.now() < deadline) {
      if (isStoppedState(current?.state)) { rememberSpace(path,current); finishTrail(path,labels); const view = spaceView(current); setStateForPath(path,view.text,view.type); return current; }
  ...
  async function stopOne(path,openConfigOnMissing=false) {
    if (!getTokenFor(path)) { const {card} = tokenUi(path); if (openConfigOnMissing) card.open = true; setStateForPath(path,'● Token do GitHub não configurado','bad'); return false; }
    const labels = ['Localizado','Solicitado','Encerrando','Desligado'];
    setBusy(path,true,'stop'); paintTrail(path,labels,0,-1);
    try {
      setStateForPath(path,'Localizando Codespace…','working');
      const space = await getSpace(path);
      if (!space) { if (openConfigOnMissing) pcConfigCard.open = true; throw new Error('Codespace ainda não foi criado.'); }
      rememberSpace(path,space); paintTrail(path,labels,1,0);
      if (isStoppedState(space.state)) { finishTrail(path,labels); const view = spaceView(space); setStateForPath(path,view.text,view.type); return true; }
      ...
      setStateForPath(path,`● ${message}`,'bad'); failTrail(path,labels,2); return false;
    } finally { setBusy(path,false); refreshConfig(); }
  }
